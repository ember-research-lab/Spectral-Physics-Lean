...
private theorem P4_hodge (p dim : ℕ) : HodgeFaithful p dim := by
  constructor
  intro α hα
  refine ⟨⟨α⟩, ?_⟩
  have h : integrationPairing (⟨α⟩ : AlgCycleClass p dim) α = hodgeNormSq α := rfl
  rw [h]; exact ne_of_gt (hodgeNormSq_pos hα)

/--
  **PROVABLE — was an axiom until 2026-09-10 (soundness census, CITE-PROVABLE).**
  Provable because `AlgCycleClass` is unrestricted, so `HodgeFaithful` is
  trivially true; this is not Lefschetz (1,1). Do not cite it as such.
...
-/
theorem lefschetz_1_1 (dim : ℕ) : HodgeFaithful 1 dim := P4_hodge 1 dim
